...
import { lintThemePackage } from "@codextheme/runtime";
...
test("private source accepts the exact package generated by the website", async () => {
  ...
  assert.ok(lintThemePackage(bundle).length > 0, "The production selector contract must exercise lint diagnostics.");
  const sha256 = createHash("sha256").update(serialized).digest("hex");
  const source = createPrivateThemeSource({
    origin: "https://codextheme.test",
    fetchImpl: async () => new Response(serialized, {
      headers: {
        "content-type": "application/vnd.codextheme.theme+json",
        "x-codextheme-sha256": sha256,
      },
  ...
  assert.equal(result.bundle.theme.displayName, "Private Custom Skin");
});

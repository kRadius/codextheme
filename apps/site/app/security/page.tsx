...
export default function SecurityPage() { return <div className="site-shell"><SiteHeader /><main className="subpage section-wrap"><article className="article-page"><Link className="back-link" href="/">← 返回首页</Link><p className="eyebrow"><span /> SECURITY MODEL</p><h1>安全边界</h1><p className="article-lead">复制命令前，你应该清楚它会做什么，也应该清楚它绝不会做什么。</p>
  ...
  <section className="article-block"><h2>固定版本与公开来源</h2><p>页面命令固定到 @codextheme/cli@0.1.1。源码公开，runtime 保留 CodeDrobe Core 0.3.0 的 Apache-2.0 许可、Git 历史、NOTICE 与精确来源记录。</p></section>
  <section className="article-block"><h2>恢复官方外观</h2><p>运行同一固定版本提供的 restore：</p><code className="inline-code">npx --yes @codextheme/cli@0.1.1 restore</code><p>恢复会处理当前 renderer 与 Codex 外观设置；如果只完成了部分恢复，活动状态会保留并返回明确错误。</p></section>
  <section className="article-block"><h2>需要帮助</h2><p>安装或兼容问题，请联系 <a className="article-link" href={`mailto:${SUPPORT_EMAIL}`}>{SUPPORT_EMAIL}</a>。不要发送对话、token、个人路径或其他私密信息。</p></section>
